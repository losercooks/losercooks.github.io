...
这篇不是泛泛的“怎么部署”，而是把我这个仓库（`losercooks.github.io`）从本地推到 GitHub Pages 的过程写清楚，重点放在**版本一致性**与**容易踩的坑**：GitHub Pages 默认 Jekyll 版本、`github-pages` gem、Ruby 升级、网络导致的 Bundler 超时、以及 `_config.yml` 的 `url/baseurl/permalink` 怎么配才不会 404。
...
这样你本地 `bundle exec jekyll build` 用的就是 Pages 这条“版本线”。
